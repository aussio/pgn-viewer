            ...
            },
            {
                ...
                variations: [
                    [
                        {
                            moveNumber: 2,
                            notation: { notation: 'f4' },
                            variations: [],
                            turn: 'w'
                        },
                        {
                            moveNumber: null,
                            notation: { notation: 'exf4' },
                            variations: [],
                            turn: 'b'
                        }
                    ]
...
describe('moveTreeFromPgn', () => {
    it('creates a MoveTree from a simple parsed PGN', () => {
        const tree = moveTreeFromPgn(parsedSimple);
        expect(tree.root.fen).toBeDefined();
        expect(tree.root.children.length).toBe(1);
        const e4 = tree.root.children[0];
        expect(e4.move.notation).toBe('e4');
        expect(e4.fen).toBe('rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq - 0 1'); // FEN after e4
        const e5 = e4.children[0];
        expect(e5.move.notation).toBe('e5');
        expect(e5.fen).toBe('rnbqkbnr/pppp1ppp/8/4p3/4P3/8/PPPP1PPP/RNBQKBNR w KQkq - 0 2'); // FEN after e4 e5
        const Nf3 = e5.children[0];
        expect(Nf3.move.notation).toBe('Nf3');
        expect(Nf3.fen).toBe('rnbqkbnr/pppp1ppp/8/4p3/4P3/5N2/PPPP1PPP/RNBQKB1R b KQkq - 1 2'); // FEN after e4 e5 Nf3
        const Nc6 = Nf3.children[0];
        expect(Nc6.move.notation).toBe('Nc6');
        expect(Nc6.fen).toBe('r1bqkbnr/pppp1ppp/2n5/4p3/4P3/5N2/PPPP1PPP/RNBQKB1R w KQkq - 2 3'); // FEN after e4 e5 Nf3 Nc6
    });

    it('throws if no game is present', () => {
        expect(() => moveTreeFromPgn([])).toThrow();
        expect(() => moveTreeFromPgn(null)).toThrow();
    });

    it('handles variations as children', () => {
        const tree = moveTreeFromPgn(parsedWithVariation);
        const e4 = tree.root.children[0];
        expect(e4.move.notation).toBe('e4');
        const e5 = e4.children[0];
        expect(e5.move.notation).toBe('e5');
        // After e5, children should be Nf3 (mainline) and f4 (variation)
    ...
    it('ensures children arrays are always present', () => {
        const parsed = [
            {
                moves: [
                    { moveNumber: 1, notation: { notation: 'e4' }, variations: [] }
                ]
            }
        ];
        const tree = moveTreeFromPgn(parsed);
                ...
                moves: [
                    {
                        ...
                        variations: [
                            [
                                { moveNumber: 1, notation: { notation: 'c4' }, variations: [] }
                            ]
                        ]
                    },
                    {
